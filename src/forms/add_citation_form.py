...
class CitationForm(FlaskForm):
    ...
    booktitle = StringField('booktitle', validators=[Optional(), Length(min=4)],
        render_kw={'placeholder': 'Book title...'}, default="")
    journal = StringField('journal', validators=[Optional(), Length(min=4)],
        render_kw={'placeholder': 'Journal...'})
    volume = IntegerField('volume', validators=[Optional()],
        render_kw={'placeholder': 'Volume...'})
    pages = StringField('pages', validators=[Optional(), Length(min=4)],
        render_kw={'placeholder': 'Pages...'})
    publisher = StringField('publisher', validators=[Optional(), Length(min=4)],
